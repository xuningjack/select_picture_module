...
import com.wuba.bangbang.uicomponents.pulltorefresh.internal.ViewCompat;
...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout
    ...
    final int LAYER_TYPE_HARDWARE = 0X2;
    final int LAYER_TYPE_NONE = 0X0;
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs,
                R.styleable.pull_to_refresh);

        if (a.hasValue(R.styleable.pull_to_refresh_ptr_mode)) {
            mMode = Mode.mapIntToValue(a.getInteger(
                    R.styleable.pull_to_refresh_ptr_mode, 0));
        }

        if (a.hasValue(R.styleable.pull_to_refresh_ptr_animation_style)) {
            mLoadingAnimationStyle = AnimationStyle.mapIntToValue(a.getInteger(
                    R.styleable.pull_to_refresh_ptr_animation_style, 0));
        }
         ...
         * Styleables from XML
         */
        if (a.hasValue(R.styleable.pull_to_refresh_ptr_refreshable_view_background)) {
            Drawable background = a
                    .getDrawable(R.styleable.pull_to_refresh_ptr_refreshable_view_background);
            if (null != background) {
                mRefreshableView.setBackgroundDrawable(background);
            }
        } else if (a
                .hasValue(R.styleable.pull_to_refresh_ptr_adapter_view_background)) {
            Utils.warnDeprecation("ptrAdapterViewBackground",
                    "ptrRefreshableViewBackground");
            Drawable background = a
                    .getDrawable(R.styleable.pull_to_refresh_ptr_adapter_view_background);
            if (null != background) {
                mRefreshableView.setBackgroundDrawable(background);
            }
        }

        if (a.hasValue(R.styleable.pull_to_refresh_ptr_over_scroll)) {
            mOverScrollEnabled = a.getBoolean(
                    R.styleable.pull_to_refresh_ptr_over_scroll, true);
        }

        if (a.hasValue(R.styleable.pull_to_refresh_ptr_scrolling_while_refreshing_enabled)) {
            mScrollingWhileRefreshingEnabled = a
                    .getBoolean(
                            R.styleable.pull_to_refresh_ptr_scrolling_while_refreshing_enabled,
                            false);
        }
    ...
    public static enum AnimationStyle {
        ...
        LoadingLayout createLoadingLayout(Context context, Mode mode,
                TypedArray attrs) {
            return new IMLoadingLayout(context, mode, attrs);
        }
    }
